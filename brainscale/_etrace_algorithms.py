...
from .typing import (PyTree, Outputs, WeightID,
...
class ETraceAlgorithmForVJP(ETraceAlgorithm):
  """
  ...
  def __init__(self,
               ...
               decay: float = None,
               num_rank: int = None):
    super().__init__(model_or_graph, decay=decay, num_rank=num_rank)
  ...
  def update_model_and_etrace(self, *args) -> Any:
    ...
    # etrace data
    etrace_vals = self._get_etrace_data()
    # update all states
    ...
    out, hidden_vals, other_vals, etrace_vals = self._true_update_fun(args,
                                                                      weight_vals,
                                                                      hidden_vals,
                                                                      other_vals,
                                                                      etrace_vals)
    ...
    self._assign_etrace_data(etrace_vals)
    return out
  ...
  def _update(self, inputs, weight_vals, hidden_vals, othstate_vals, etrace_vals,
    ...
    out, hidden_vals, othstate_vals, xs, dfs = self.graph.solve_spatial_gradients(*inputs)
    xs = jax.lax.stop_gradient(xs)
    dfs = jax.lax.stop_gradient(dfs)

    # eligibility trace update
    etrace_vals = self._update_etrace_data(temporal_grads, xs, dfs)
  ...
  def _update_fwd(self, args, weight_vals, hidden_vals, othstate_vals, etrace_vals,
    ...
    # spatial gradients of the weights
    out, hiddens, oth_states, xs, dfs, residuals = self.graph.solve_spatial_gradients_and_vjp_jaxpr(*args)
    xs = jax.lax.stop_gradient(xs)
    dfs = jax.lax.stop_gradient(dfs)
    ...
    # returns
    id2weight_val = {id(st): val for st, val in zip(self.weight_states, weight_vals)}
    return (out, hiddens, oth_states, etrace_vals), (residuals, etrace_vals, id2weight_val)
  ...
  def _update_bwd(self, fwd_res, grads) -> Tuple[dG_Inputs, dG_Weight, dG_Hidden, dG_State, None]:
    ...
    dg_weights = self._solve_weight_gradients(etrace_xs,
                                              etrace_dfs,
                                              dg_perturbs,
                                              id2weight_val,
  ...
  def _solve_weight_gradients(self, xs, dfs, hiddens, id2weight_val, dg_non_etrace_params):
    ...
    for relation in self.graph.weight_hidden_relations:
      ...
      for i, hid_var in enumerate(relation.hidden_vars):
        ...
        df_hid = df * relation.hidden2df[i](hiddens[hid_var])
        weight_id = id(relation.weight)
        dg_weight = weight_op_gradient(relation.op_jaxpr, x, id2weight_val[weight_id], df_hid)
  ...
  def _solve_temporal_gradients(self, *args):
    raise NotImplementedError

  def _update_etrace_data(self, temporal_grads, xs, dfs) -> EtraceVals:
    raise NotImplementedError
...
def _diag_hidden_update(self: ETraceAlgorithm, hiddens, others, *params):
  ...
  with stop_param_gradients():
    self.graph.call_org_model(*params)
  hiddens = [st.value for st in self.hidden_states]
  return hiddens


def _solve_diag_jacobian(self: ETraceAlgorithm, *args) -> Dict[jax.core.Var, jax.Array]:
  # [compute D^t]
...
class ExpSmDiagOnAlgorithm(ETraceAlgorithmForVJP):
  """
  ...
  def _solve_temporal_gradients(self, *args) -> Dict[jax.core.Var, jax.Array]:
    return _solve_diag_jacobian(self, *args)

  def _update_etrace_data(self,
                          temporal_grads: Dict[jax.core.Var, jax.Array],
                          xs: Dict[jax.core.Var, jax.Array],
                          dfs: Dict[jax.core.Var, jax.Array]) -> EtraceVals:
    # the weight x
    etrace_xs = dict()
    for x in self.etrace_xs:
      etrace_xs[x] = low_pass_filter(self.etrace_xs[x].value, xs[x], self.decay)

    # the weight df * diagonal
    etrace_dfs = dict()
    for dfkey in self.etrace_dfs:
      df_var, state_var = dfkey
      etrace_dfs[dfkey] = self.etrace_dfs[dfkey].value * temporal_grads[state_var]

    # the weight df
    for dfkey in self.etrace_dfs:
      df_var, state_var = dfkey
      etrace_dfs[dfkey] = expon_smooth(etrace_dfs[dfkey], dfs[df_var], self.decay)
...
class DiagOn2Algorithm(ETraceAlgorithmForVJP):
  """
  ...
  """
  etrace_bwg: Dict[Tuple[WeightID, StateJaxVar], bc.State]  # batch of weight gradients

  def init_state(self, batch_size: int = None, *args, **kwargs):
    init = bt.init.ZeroInit()
    # The states of batched weight gradients:
    self.etrace_bwg = bc.visible_state_dict()
    for relation in self.graph.weight_hidden_relations:
      for statevar in relation.hidden_vars:
        key = (id(relation.weight), statevar)
        if key in self.etrace_bwg:
          raise ValueError(f'The relation {key} has been added. ')
        val = jax.tree.map(init, relation.weight.value)
        self.etrace_bwg[key] = bc.State(val)
  ...
  def _update_etrace_data(self,
    ...
    etrace_vals = dict()
    for x in self.etrace_xs:
      etrace_vals[x] = low_pass_filter(self.etrace_xs[x].value, xs[x], self.decay)
