...
import jax.typing
from braintools import functional as F
...
class Threshold(bc.Module, ElementWiseBlock):
...
class ReLU(bc.Module, ElementWiseBlock):
  ...
  Examples::
    ...
    An implementation of CReLU - https://arxiv.org/abs/1603.05201
      ...
      >>> import braincore as bc
      >>> m = nn.ReLU()
      >>> x = bc.random.randn(2).unsqueeze(0)
...
class RReLU(bc.Module, ElementWiseBlock):
...
class Hardtanh(bc.Module, ElementWiseBlock):
...
class Sigmoid(bc.Module, ElementWiseBlock):
  r"""Applies the element-wise function:
...
class Hardsigmoid(bc.Module, ElementWiseBlock):
...
class Tanh(bc.Module, ElementWiseBlock):
...
class Mish(bc.Module, ElementWiseBlock):
...
class Hardswish(bc.Module, ElementWiseBlock):
...
class ELU(bc.Module, ElementWiseBlock):
...
class CELU(bc.Module, ElementWiseBlock):
  r"""Applies the element-wise function:
...
class SELU(bc.Module, ElementWiseBlock):
...
class GLU(bc.Module, ElementWiseBlock):
...
class GELU(bc.Module, ElementWiseBlock):
...
class Hardshrink(bc.Module, ElementWiseBlock):
...
class LeakyReLU(bc.Module, ElementWiseBlock):
  r"""Applies the element-wise function:
...
class LogSigmoid(bc.Module, ElementWiseBlock):
  r"""Applies the element-wise function:
...
class Softplus(bc.Module, ElementWiseBlock):
...
class Softshrink(bc.Module, ElementWiseBlock):
...
class PReLU(bc.Module, ElementWiseBlock):
  r"""Applies the element-wise function:
  ...
  def __init__(self, num_parameters: int = 1, init: float = 0.25, dtype=None) -> None:
    ...
    super().__init__()
    self.weight = bc.ParamState(jnp.ones(num_parameters, dtype=dtype) * init)
...
class Softsign(bc.Module, ElementWiseBlock):
  r"""Applies the element-wise function:
...
class Tanhshrink(bc.Module, ElementWiseBlock):
  r"""Applies the element-wise function:
...
class Softmin(bc.Module, ElementWiseBlock):
...
class Softmax(bc.Module, ElementWiseBlock):
...
class Softmax2d(bc.Module, ElementWiseBlock):
...
class LogSoftmax(bc.Module, ElementWiseBlock):
...
class Identity(bc.Module, ElementWiseBlock):
...
class Dropout(bc.Module, ElementWiseBlock):
  """A layer that stochastically ignores a subset of inputs each training step.

  In training, to compensate for the fraction of input values dropped (`rate`),
  all surviving values are multiplied by `1 / (1 - rate)`.

  This layer is active only during training (``mode=braincore.mixin.Training``). In other
...
class _DropoutNd(bc.Module, ElementWiseBlock):
  __module__ = 'brainscale'
  prob: float
  channel_axis: int
  minimal_dim: int
  ...
  def __call__(self, x):
    ...
    # generate mask
    if fit_phase:
      keep_mask = jnp.broadcast_to(bc.random.bernoulli(self.prob, mask_shape), x.shape)
      return jnp.where(keep_mask,
                       jnp.asarray(x / self.prob, dtype=dtype),
                       jnp.asarray(0., dtype=dtype))
    else:
...
class Dropout3d(_DropoutNd):
  ...
  Examples::
      ...
      >>> m = Dropout3d(p=0.2)
      >>> x = bc.random.randn(20, 16, 4, 32, 32)
      >>> output = m(x)
...
class SpikeBitwise(bc.Module, ElementWiseBlock):
