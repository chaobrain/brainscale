...
def hid_group_key(i: int) -> str:
    return f'hidden_group_{i}'
...
class ETraceVjpAlgorithm(ETraceAlgorithm):
    ...
    def __init__(
        ...
        # graph
        graph = ETraceGraphVJP(model, vjp_method=vjp_method)
...
def _init_IO_dim_state(
    ...
    if not isinstance(relation.weight, ElemWiseParam):
        if relation.x not in etrace_xs:
            ...
            etrace_xs[relation.x] = EligibilityTraceState(u.math.zeros(shape, dtype))
    ...
    for group in relation.hidden_groups:
        ...
        key = (relation.y, hid_group_key(i=group.index))
        if key in etrace_dfs:  # relation.y is an unique output of the weight operation
        ...
        etrace_dfs[key] = EligibilityTraceState(u.math.zeros(shape, y_dtype))
...
def _update_IO_dim_etrace_scan_fn(
    ...
    #
    hid_group_jacobians: Sequence[HiddenGroupJacobian] = jacobians[2]
    ...
    for relation in hid_weight_op_relations:
        ...
        for group in relation.hidden_groups:
            ...
            #
            df_key = (relation.y, hid_group_key(group.index))
    ...
    def init_etrace_state(self, *args, **kwargs):
        ...
        for relation in self.compiled.hidden_param_op_relations:
            ...
            if relation.x not in self.etrace_xs:
                ...
                self.etrace_xs[relation.x] = EligibilityTraceState(u.math.zeros(shape, dtype))
            ...
            for group in relation.hidden_groups:
                ...
                for hidden_outvar in group.hidden_outvars:
                    ...
                    self.etrace_dfs[key] = EligibilityTraceState(u.math.zeros(shape, dtype))
    ...
    for group in relation.hidden_groups:
        ...
        for hid_path in group.hidden_paths:
            ...
            if key in self.etrace_bwg:  # The key should be unique
                raise ValueError(f'The relation {key} has been added. ')
            self.etrace_bwg[key] = EligibilityTraceState(
...
def _update_param_dim_etrace_scan_fn(
    ...
    for relation in hidden_param_op_relations:
        ...
        if not isinstance(relation.weight, ETraceParam):
            raise NotImplementedError(
                f'When using {ParamDimVjpAlgorithm.__name__} or '
                f'{HybridDimVjpAlgorithm.__name__} algorithms, '
...
def _solve_param_dim_weight_gradients(
    ...
    for relation in weight_hidden_relations:
        if not isinstance(relation.weight, ETraceParam):
            raise NotImplementedError(
                f'When using {ParamDimVjpAlgorithm.__name__} '
                f'or {HybridDimVjpAlgorithm.__name__} algorithms, '
                f'the weight should be an {ETraceParam.__name__}. '
            )
