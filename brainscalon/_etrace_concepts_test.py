...
class TestETraceTreeState(unittest.TestCase):
    def test_init(self):
        ...
        state = brainscalon.ETraceTreeState(value)
...
#         op = brainscalon.ETraceOp(lambda x, w: x + w)
#         param = brainscalon.ETraceParam(weight, op)
#         self.assertEqual(param.gradient, brainscalon.ETraceGrad.adaptive)
#
...
class TestElemWiseParam(unittest.TestCase):
    def test_init(self):
        weight = bst.random.randn(10, 10)
        op = brainscalon.ElemWiseOp(lambda w: w)
        param = brainscalon.ElemWiseParam(weight, op)
...
class TestNonTempParam(unittest.TestCase):
    def test_init(self):
        weight = bst.random.randn(10, 10)
        op = lambda x, w: x + w
        param = brainscalon.NonTempParam(weight, op)
...
class TestFakeETraceParam(unittest.TestCase):
    def test_init(self):
        weight = bst.random.randn(10, 10)
        op = lambda x, w: x + w
        param = brainscalon.FakeETraceParam(weight, op)
        self.assertEqual(param.value.shape, weight.shape)
...
class TestFakeElemWiseParam(unittest.TestCase):
    def test_init(self):
        weight = bst.random.randn(10, 10)
        op = brainscalon.ElemWiseOp(lambda w: w)
        param = brainscalon.FakeElemWiseParam(weight, op)
        self.assertEqual(param.value.shape, weight.shape)
