...
class TestETraceVjpGraphExecutor(unittest.TestCase):
    ...
    def setUp(self):
        self.model = brainscalon.nn.GRUCell(self.in_size, 4)
        bst.nn.init_all_states(self.model)
        bst.environ.set(dt=0.1 * u.ms)

    def test_initialization(self):
        executor = brainscalon.ETraceVjpGraphExecutor(self.model)
        self.assertEqual(executor.vjp_method, 'single-step')

        executor = brainscalon.ETraceVjpGraphExecutor(self.model, vjp_method='multi-step')
        self.assertEqual(executor.vjp_method, 'multi-step')

    def test_invalid_vjp_method(self):
        with self.assertRaises(AssertionError):
            brainscalon.ETraceVjpGraphExecutor(self.model, vjp_method='invalid')
    ...
    def test_single_step_vs_multi_step(self):
        single_step_executor = brainscalon.ETraceVjpGraphExecutor(self.model, vjp_method='single-step')
        multi_step_executor = brainscalon.ETraceVjpGraphExecutor(self.model, vjp_method='multi-step')
