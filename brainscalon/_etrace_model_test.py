...
class IF_Delta_Dense_Layer(bst.nn.Module):
    def __init__(
        ...
        super().__init__()
        self.neu = brainscalon.nn.IF(n_rec, tau=tau_mem, spk_reset=spk_reset, V_th=V_th)
        w_init = u.math.concatenate([ff_init([n_in, n_rec]), rec_init([n_rec, n_rec])], axis=0)
        self.syn = brainscalon.nn.Linear(n_in + n_rec, n_rec, w_init=w_init * u.mA, b_init=bst.init.ZeroInit(unit=u.mA))
    ...
    def __init__(
        ...
        if input_ei_sep:
            ...
            self.exe_syn = bst.nn.AlignPostProj(
                comm=brainscalon.nn.SignedWLinear(self.n_exc_in + self.n_exc_rec, n_rec, w_init=weight),
                syn=brainscalon.nn.Expon.desc(n_rec, tau=tau_syn),
            ...
            self.inh_syn = bst.nn.AlignPostProj(
                comm=brainscalon.nn.SignedWLinear(self.n_inh_in + self.n_inh_rec, n_rec, w_init=weight),
                syn=brainscalon.nn.Expon.desc(n_rec, tau=tau_syn),
            ...
            self.inp_syn = bst.nn.AlignPostProj(
                comm=brainscalon.nn.Linear(n_in, n_rec, w_init=ff_init([n_in, n_rec]) * u.mS, b_init=b_init),
                syn=brainscalon.nn.Expon.desc(n_rec, tau=tau_syn),
...
class LIF_ExpCo_Dense_Layer(_ExpCo_Dense_Layer):
    ...
    def __init__(
        ...
        neu = brainscalon.nn.LIF(n_rec, tau=tau_mem, spk_fun=spk_fun, spk_reset=spk_reset, V_th=V_th)
...
class LIF_STDExpCu_Dense_Layer(bst.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.neu = brainscalon.nn.LIF(n_rec, tau=tau_mem, spk_fun=spk_fun, spk_reset=spk_reset, V_th=V_th)
        self.std = brainscalon.nn.STD(n_rec, tau=tau_std, U=0.1)
        if inp_std:
            self.std_inp = brainscalon.nn.STD(n_in, tau=tau_std, U=0.1)
        else:
            self.std_inp = None
...
class LIF_STPExpCu_Dense_Layer(bst.nn.Module):
    def __init__(
        ...
        self.neu = brainscalon.nn.LIF(n_rec, tau=tau_mem, spk_fun=spk_fun, spk_reset=spk_reset, V_th=V_th)
        self.stp = brainscalon.nn.STP(n_rec, tau_f=tau_f, tau_d=tau_d)
        if inp_stp:
            self.stp_inp = brainscalon.nn.STP(n_in, tau_f=tau_f, tau_d=tau_d)
...
class ALIF_Delta_Dense_Layer(bst.nn.Module):
    ...
    def __init__(
        ...
        self.syn = bst.nn.DeltaProj(
            comm=brainscalon.nn.Linear(n_in + n_rec, n_rec, w_init=w_init * u.mV, b_init=bst.init.ZeroInit(unit=u.mV)),
            post=self.neu
        )
...
class ALIF_STDExpCu_Dense_Layer(bst.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.neu = brainscalon.nn.ALIF(n_rec, tau=tau_mem, spk_fun=spk_fun, spk_reset=spk_reset, V_th=V_th, beta=beta)
        self.std = brainscalon.nn.STD(n_rec, tau=tau_std, U=0.1)
        if inp_std:
            self.std_inp = brainscalon.nn.STD(n_in, tau=tau_std, U=0.1)
        else:
