...
class TestCompileGraphRNN(unittest.TestCase):
    def test_gru_one_layer(self):
        ...
        gru = brainscalon.nn.GRUCell(n_in, n_out)
        bst.nn.init_all_states(gru)

        input = bst.random.rand(n_in)
        graph = brainscalon.compile_etrace_graph(gru, input, include_hidden_perturb=False)

        self.assertTrue(isinstance(graph, brainscalon.ETraceGraph))
    ...
    def test_lru_one_layer(self):
        ...
        lru = brainscalon.nn.LRUCell(n_in, n_out)
        bst.nn.init_all_states(lru)

        input = bst.random.rand(n_in)
        graph = brainscalon.compile_etrace_graph(lru, input, include_hidden_perturb=False)
    ...
    def test_lstm_one_layer(self):
        ...
        lstm = brainscalon.nn.LSTMCell(n_in, n_out)
        bst.nn.init_all_states(lstm)

        input = bst.random.rand(n_in)
        graph = brainscalon.compile_etrace_graph(lstm, input, include_hidden_perturb=False)
        ...
        hid_states = lstm.states(brainscalon.ETraceState)
    ...
    def test_lstm_two_layers(self):
        ...
        net = bst.nn.Sequential(
            brainscalon.nn.LSTMCell(n_in, n_out),
            bst.nn.ReLU(),
            brainscalon.nn.LSTMCell(n_out, n_in),
        )
        bst.nn.init_all_states(net)

        input = bst.random.rand(n_in)
        graph = brainscalon.compile_etrace_graph(net, input, include_hidden_perturb=False)
    ...
    def test_lru_two_layers(self):
        ...
        net = bst.nn.Sequential(
            brainscalon.nn.LRUCell(n_in, n_out),
            bst.nn.ReLU(),
            brainscalon.nn.LRUCell(n_in, n_out),
        )
        bst.nn.init_all_states(net)
