...
@bst.transform.jit(static_argnums=2)
...
def f_predict(inputs):
  bst.init_states(model, batch_size=inputs.shape[1])
  return bst.transform.for_loop(model, inputs)
...
def f_train(inputs, targets):
  grads, l = bst.transform.grad(f_loss, grad_vars=weights, return_value=True)(inputs, targets)
  opt.update(grads)
  return l
...
plt.plot(bst.math.as_numpy(y[:, 0]).flatten(), label='Ground Truth')
plt.plot(bst.math.as_numpy(predicts[:, 0]).flatten(), label='Prediction')
plt.legend()
plt.show()
